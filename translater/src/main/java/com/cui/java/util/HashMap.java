 ...
 * 容量是指哈希表里的桶的数量，初始容量就是这个哈希表被创造时的容量。
 *
 ...
 * 哈希表开始重散列（rehash，也是就说，内部的数据结构将重构）来翻倍哈希表中的桶数量
 *
 * <p>As a general rule, the default load factor (.75) offers a good tradeoff between time and space costs.
 * 一般而言，默认的加载因子(.75)很好地权衡了时间和空间的消耗。
 *
 * Higher values decrease the space overhead but increase the lookup cost
 * 更大的值减少了空间的开销但是增加了查询的损耗
 *
 * (reflected in most of the operations of the <tt>HashMap</tt> class, including <tt>get</tt> and <tt>put</tt>).
 * （这将会影响大多数HashMap的操作，包括get&put）。
 *
 ...
 * 如果没有这种对象存在，那么map需要用Collections.synchronizedMap方法来包装，
 *
 ...
 * 这一步最好在创建map时完成，以免意外的非同步访问map；
 *
 * <p>The iterators returned by all of this class's "collection view methods" are <i>fail-fast</i>:
 * 这个类的所有“集合视图方法”反正的迭代器都是fail-fast
 *
 * if the map is structurally modified at any time after the iterator is created,
 * in any way except through the iterator's own <tt>remove</tt> method, the iterator will throw a {@link ConcurrentModificationException}.
 * 如果创建迭代器以后再对map作结构性修改，除了通过迭代器自带的remove方法以外，
 * 都会导致ConcurrentModificationException异常。
 *
 ...
 * 因此，对于同时修改，迭代器会快速而明确的报错，而不是在未来某个不确定的时刻冒险。
 *
 ...
 * 注意，迭代器fail-fast行为并不保证被严格执行，也就是说，很难在存在非同步，同时修改时保证fail-fast。
 *
 * Fail-fast iterators throw <tt>ConcurrentModificationException</tt> on a best-effort basis.
 * Fail-fast迭代器会在最大努力的基础上，抛出ConcurrentModificationException异常。
 *
 ...
 * 因此，编写一个依赖于这个异常的正确性的程序是错误的：fail-fast行为，只能被用来探测bug。
 *
...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     * they are transformed into bins of TreeNodes, each structured similarly to those in java.util.TreeMap.
     * map通常是一个以bin（bucketed）为基础的哈希表，但当bin变得过大时，他们会转化为java.util.TreeMap的树形节点结构。
     *
